...
<div class="container">
    <div class="form-container">
        ...
        $sql = "UPDATE bicicletas SET Marca = ?, Modelo = ?, Tipo = ?, Precio = ?, Stock = ?, ID_Cliente = ? WHERE ID_Bicicleta = ?";
        $stmt = $mysqli->prepare($sql);
        $stmt->bind_param("sssiiii", $marca, $modelo, $tipo, $precio, $stock, $id_cliente, $id);

        if ($stmt->execute()) {
            $mensaje = "Bicicleta actualizada con éxito.";
            $clase = "success";
        } else {
            $mensaje = "Error al actualizar la bicicleta.";
            $clase = "danger";
        }
        ?>
